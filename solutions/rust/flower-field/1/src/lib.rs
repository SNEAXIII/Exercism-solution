...
impl<'a> Garden<'a> {
    ...
    Self{row_count,column_count,counter,garden}
    }
    ...
    fn solve(&mut self) -> Vec<String> {
        ...
        let mut result: Vec<String> = Vec::with_capacity(self.row_count);
        result.resize(self.row_count, String::with_capacity(self.column_count));
                for (iy,row) in self.garden.iter().enumerate() {
            for ix in 0..row.len() {
                let flattened_index = self.get_flat_index(ix,iy);
                if self.counter[flattened_index] == 0 {
                    result[iy].push(' ');
                } else if self.counter[flattened_index] == -1 {
                    result[iy].push('*');
                } else {
                    result[iy].push((self.counter[flattened_index] as u8 + b'0') as char)
                }
            }
        }
        result
