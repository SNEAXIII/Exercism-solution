...
#[must_use]
fn recurse_fill(stack: &mut Stack, next_domino: &[Stack], dominos: &[Domino]) -> bool {
    ...
    for domino in &next_domino[current_value] {
        ...
        if recurse_fill(stack, next_domino, dominos) {
            return true;
        }
        stack.pop();
    }
...
pub fn chain(dominos: &[Domino]) -> Option<Vec<Domino>> {
    ...
    if recurse_fill(&mut stack, &next_domino, dominos) {
        ...
        // todo check earlier
        if result[0].0 == result.last().unwrap().1 {
            Some(result)
        } else {
            None
        }
