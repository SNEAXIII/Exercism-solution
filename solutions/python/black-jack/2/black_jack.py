...
def value_of_card(card: str):
    ...
    if card == ACE:
        return 1
    if card in FACE_CARD_CONVERTER:
        return FACE_CARD_CONVERTER[card]
...
def is_blackjack(card_1: str, card_2: str):
    is_contain_10_power_card = max(
        value_of_card(card_1), value_of_card(card_2)) == 10
    is_contain_ace_card = ACE in (card_1, card_2)
    return is_contain_10_power_card and is_contain_ace_card
