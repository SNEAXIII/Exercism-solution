...
class BowlingGame:
    ...
    def _third_throw(self, pins, actual_frame):
        ...
        temp_score = actual_score + pins
        print(actual_frame)
        print(f"{temp_score > PIN_NUMBER = } , {actual_frame[0] == STRIKE = } , {actual_frame[1] != STRIKE = }")
    ...
    def next_frame(self):
        if self.is_last_frame():
            ...
            try:
                is_third_shot_is_possible = (final_frame[1] in (
                    SPARE, STRIKE) and throw_count == 2) or (final_frame[0] == STRIKE and throw_count == 2)
            except IndexError:
                pass
    ...
    def roll(self, pins: int):
        ...
        match number_of_throw:
            case 0:
                self._first_throw(pins, actual_frame)
            case 1:
                self._second_throw(pins, actual_frame)
            case 2:
                self._third_throw(pins,actual_frame)
    ...
    def score(self):
        ...
        for index in range(len(flat_list)-3, -1, -1):
            ...
            if number == SPARE:
                ...
                final_list[index] = PIN_NUMBER-tail+head
            elif number == STRIKE:
                flat_list[index] = PIN_NUMBER
                final_list[index] = PIN_NUMBER+head+next_head
